...
class ExportOfflineList extends BaseSchedule
{
    ...
    public function handle()
    {
        ...
        $parts     = $this->getParts();
        $report    = [];
        $report[]  = ['订单日期', '订单时间', '订单号', '学校ID', '学校名称', '省', '市', '区县', '市场专员', '加盟校', '合同档', '协议价', '昵称', '备注名', '学生手机', '金额', '补登', '天数', '计数'];
        $orders    = \DB::table('order_offline')
            ->selectRaw('order_offline.created_at, order_offline.id as offline_id, school.id, school.name, school.marketer_id, nickname, group_concat(DISTINCT vanclass_student.mark_name) as _mark_name, user.phone, pay_fee, days, approval_code')
        ...
        $file     = $this->store($filename, public_path('exports').'/offline', $report);
