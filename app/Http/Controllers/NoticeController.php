...
class NoticeController extends Controller
{
    public function lists()
    {
        ...
        $notices = $this->setModel('notice')->where(function ($query) use ($user_id) {
        ...
        $users = $this->setModel('account')->select(['id', 'username', 'avatar'])->whereIn('id', $user_ids)->orderBy('id', 'desc')->get()->toArray();
    ...
    {
        $id = $request->get('id');
        $this->setModel('notice')->where('id', $id)->update(['has_read' => 1]);
        return 1;
    }

    public function ajaxCheck()
    {
        $notices = $this->setModel('notice')->with('sender')->where('receiver_id', $this->getUser('id'))->where('status', 1)
